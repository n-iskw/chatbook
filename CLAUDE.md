...
| ページ送りは指で触る端末だけページの下（スクロール内）。hover できる端末には出さない | `PageToolbar`（`components/PdfViewer/PageToolbar.tsx`。描くのは `AppPage`）          |
| マウスで選んだら浮遊ポップオーバー                                                   | 下端の `SelectionActionBar` →「AIに質問」で `SelectionPopover`（`floating={false}`） |
| ペイン境界のドラッグハンドルで幅を変える                                             | ハンドルは出さない（分ける相手がいない）                                             |

**この表は「何が画面に出るか」だけを決める**（ページ送りの行だけは幅と端末の能力の両方で
決まる。下記「ページ送りは指で触る端末にだけ出す」）。**「どう触れるか」は幅で分けない**——タブレットは
...
| hover の有無で出し入れするもの | CSS の hover メディアクエリ 2 箇所だけ（本棚の削除ボタンは `[@media(hover:none)]:opacity-100` で常時表示、広い画面のページ送りは `[@media(hover:hover)]:hidden` で非表示）。JS の分岐ではなく端末の能力そのものを問うため |
...
**ページ送りは指で触る端末にだけ出す。**出るところなら形は同じ `PageStepper`
（前 / 現在地 / 次。`components/PdfViewer/PageStepper.tsx`）で、置き場所だけが違う——狭い画面は
`PageToolbar` が窓の下端に留め、広い画面は `PdfViewer` がページの下（スクロール内）に置く。
**hover できる端末ではその行を出さない**（`PdfViewer.tsx` のラッパーの
`[@media(hover:hover)]:hidden`）——ページを送るのは端のクリックと `h` / `l` で足り、
1 ページずつ送って読む本で今が何ページ目かを知っても使いようがない。
**幅ではなく端末の能力で分ける**のは、タブレットが広い画面に入るため（1024px は
`NARROW_MAX_WIDTH` の上）で、幅で消すとタブレットからページ送りが消える。裏返すと、画面に
触れるノート PC は hover できるので出ない側に落ちる（マウスとキーボードがあるので困らない）。
**開閉のトグルはヘッダーに 2 つ
...
目次から飛んだとき（狭い画面のみ）・広い画面のトグル・URL の `?outline=` とサーバの
読書位置からの復元（どちらも広い画面のみ。下記「リーダーの URL は `useReadingLocation` が
単独で書く」「読んでいた場所は本と一緒に運ぶ」）。
...
リロードと共有リンクで同じ状態に戻るよう、リーダーの状態は 4 つのクエリパラメータに
...
| `outline`   | `open` / `closed`              | `outlineOpenAtom`     | 本の保存値へフォールバックする（広い画面のみ）     |

`page` / `panel` / `outline` は既定値も明示するが、`selection` の「無い」は省略で表す
（null を綴る自然な形が無いため）。**`page` / `selection` / `#:~:text=` がどれも無い＝本棚から
開いたときだけは、書き出しをサーバの読書位置が決まるまで待つ**（下記「読んでいた場所は本と
一緒に運ぶ」）。`outline` はこの「場所を名指したか」の判定には数えない——目次は場所ではなく、
ページに添えるものだから。

**`outline` だけは狭い画面で読みも書きもしない**。あちらの目次はページを覆うドロワーで、
ジャンプのたびに自分で閉じる。書かなければ既存の値が温存されるので、広い画面が選んだ状態を
スマホで畳んでしまうことがない（サーバ側の「`outlineOpen` 省略＝保持」と同じ意味論）。
`outline` を名指していないリンク（この機能より前に書かれたもの）は本の保存値へフォールバック
するので、**その決着まで `locationReady` は立たない**。Chrome の「ハイライトへのリンクをコピー」が書く `#:~:text=`
...
**目次はサーバより先に URL が持つ**（`?outline=`。上記「リーダーの URL は
`useReadingLocation` が単独で書く」）。名指されていればそれが勝ち、名指していないリンクだけが
本の保存値へフォールバックする。リロードと共有リンクが本の到着を待たずに戻るのはこのため——
待たせると、幅から決まる既定の目次が一瞬出てから畳まれ、しかもその既定が保存値を上書きする。
`outline` を名指していない古いリンクだけは決着を待つので、**着地しただけでは保存されない**
（次にページ・チャット・目次のどれかが動いたときに保存される）。
...
| hover の有無で変わる分岐（ページ送りの行）              | E2E で 2 本 1 組。出ない側は `desktop` の `keeps no row of page controls…`、出る側は `tablet` の `keeps the row of page controls…`。**片方だけでは「常に非表示」と区別できない**ので必ず対で持つ。jsdom は Tailwind の CSS を当てないので `display: none` になったかを見られない |
...
- **`desktop` は「今どのページか」をカウンタではなく描かれたページで見る**（`drawnPage` が
  ...
  出ないのでカウンタが無く、そのうえ span はページが実際に描かれて初めて現れるので観測として
  も強い（カウンタは atom が変わった瞬間に切り替わるだけ）。ページを進めるのはキーボード
  （`l` / `h`）か端のクリック。`tablet` / `mobile` は今もカウンタ（`pageLabel`）を見る
