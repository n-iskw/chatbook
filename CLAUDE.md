...
アドレスに載っている（`?page=` / `?panel=` / `?selection=` / `?outline=`）ので、別ルートへ送ると
...
| hover の有無で変わる分岐（ページ送りの行）              | E2E で 2 本 1 組（下記「hover の分岐は 2 本で 1 つの約束」）。**jsdom では触らないこと**——CSS を当てないので `display: none` を見られないだけでなく、行は広い画面でも DOM に残るので click できてしまう（実機では見えないものを触る「動いていないのに通る」テストになる） |
