...
| チャットだけを読むならヘッダーの「チャットを最大化」          | シートのグラバーで全高まで引き上げる                                                                |
...
**開閉のトグルはヘッダーに並べる**（`AppPage`）——目次を畳むのとチャットを畳むのは同じ
...
`src/front/atoms/chatAtom.ts`）。ヘッダーの 3 つ目のトグル（「チャットを最大化」/
「最大化を解除」）で、PDF ペインとハンドルを inline の `visibility: hidden` にし、チャットの
ペインを `absolute inset-0` でその上に重ねる。**ペインを外さず、幅も 0 にしない**——外すと
`useKeyboardShortcuts`（購読しているのは `PdfViewer` の中）ごと消え、幅 0 は `PdfPage` を
unmount するので戻ったときにページを描き直す。寸法を変えなければ `PdfViewer` の
ResizeObserver は何も聞かず、解除したページは 1px も動かない。代償は**見えないままでも
`←` / `→` がページを送ること**で、キーボードが生きていることの裏面として受け入れている。
**チャットのペインは同じ要素のまま**にする（2 つに分けると `ChatArea` が作り直され、開いて
いる会話・検索語・打ちかけの質問が消える）。
...
`full`＝ペインいっぱい）はどこにも保存しない。** `chatPanelOpenAtom` は「広い画面でパネルを
畳んだか」で、本と一緒にサーバへ運ぶ（下記「読んでいた場所は本と一緒に運ぶ」）。シートは毎回
`closed` から始まる——電話には畳んで残す第 2 のペインが無く、half と full は場所ではなく
ジェスチャだから。**full がペインいっぱいなのは、広い画面の最大化にあたるのがこれだから**
——上に残した数十 px のページは読めるものではなく、そのぶん回答の行数が減る。ペインは
ツールバーの上で終わるので、全高でもページ送りと閉じる操作は残る（`e2e/mobile.spec.ts` の
