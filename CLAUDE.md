...
追従しない。これは**本が何も言っていないときの既定**で、本に保存値があれば到着時にそちらへ
移る。書き手は `PageToolbar` の目次ボタン・キーボードショートカット・暗幕のタップ・
目次から飛んだとき（狭い画面のみ）・広い画面のトグル・サーバの読書位置からの復元
（広い画面のみ。下記「読んでいた場所は本と一緒に運ぶ」）。

**チャットパネルの既定は幅を見ない**（`chatPanelOpenAtom` は `src/front/atoms/chatAtom.ts` で
`atom<boolean>(true)`）。畳んで残す第 2 のペインは広い画面にしか無く、狭い画面はこの atom を
読みも書きもしない（あちらは `ChatSheet` と `chatSheetAtom`）ので、幅で分ける意味がない。
本に保存値があれば目次と同じく到着時にそちらへ移る。
...
本を待たずに押せるので、取得中に畳んだ目次が到着で開き直されると、これは直したはずの症状
そのものになる。**短くなった本に備えて最終ページで丸める**。**本が届かなければ復元も保存も
起きない**——`pendingRestore` が下りないので `locationReady` が立たず、本棚から開いた経路では
URL も書かれない（読者に見えるのは本の読み込みエラーだけなので、これで困る人はいない）。

**代償として、リロードでは本が届くまで既定の開閉が見える**（目次は幅から決まる既定、
チャットパネルは常に開）。ページそのものが本無しには描けないので短い窓だが、その間トグルは
押せる——押した結果が復元に取り消されないことは上の `panelsWhenOpened` が保証する。
...
既定の開閉を書き戻すと、復元中の場所を自分で消す。立った直後に
...
（`outlineOpen` / `chatPanelOpen` の省略＝サーバの保存値を保持）。**理由は 2 つの atom で
違う**——目次はページを覆うドロワーで、ジャンプのたびに自分で閉じるので、その値を送ると広い
画面が開いておいた目次を畳んでしまう。チャットパネルは狭い画面では動かす口が無く（あちらは
`chatSheetAtom` という別物）、触られていない既定の `true` が残っているだけなので、送れば
広い画面が畳んだ状態を上書きする。`null` は「まだ広い画面が何も表明していない」で、閉とは別物。
