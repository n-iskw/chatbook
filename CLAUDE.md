...
| hover の有無で変わる分岐（ページ送りの行）              | E2E のみ。`desktop` が出ない側（`keeps no row of page controls…`）、`tablet` / `mobile` は開始時にページ番号を待つので出る側を押さえる。jsdom は Tailwind の CSS を当てないので `display: none` になったかを見られない |
