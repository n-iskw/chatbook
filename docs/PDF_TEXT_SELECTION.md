...
- **描画品質と選択表示の一貫性** (§6〜§7) — 目には見えるが、原因が pdf.js の内部仕様や自前の確定判定にあって特定しにくい。§7 の後半は出た直後を見ないと分からない類で、末尾のコピーに至っては画面に何も現れない
...
11. **選択を自前で描くなら、コピーにも自前で答える** (項目 7 の対策を入れてから)。入力欄が奪った選択はクリップボードにも無い (→ §7)
...
**見た目を一本化した代償として、コピーが黙って壊れる。** 入力欄がフォーカスを取った時点でブラウザの選択は畳まれているので、そこで Cmd+C を押しても**クリップボードには何も書き込まれない**——前に入っていた内容がそのまま貼り付く。それでも矩形のスナップショットは選択を描き続け、`::selection` を消したせいでネイティブの選択色は元から見えないので、読者の画面では**選んだままにしか見えない**。冒頭のスナップショット対策が、そのまま症状を隠す側に回る。

対策は、**入力欄が出ている間はコピーにも自前で答える**こと。`document` の `copy` を購読し、`clipboardData.setData("text/plain", 選択文)` を書いてから `preventDefault()` する。**キャンセルが要る**——しないと UA の既定処理が「今の選択」(ここでは空) を書き戻し、`setData` の結果を捨てる。購読は入力欄の生存期間に閉じる——出ていないときの選択はブラウザのものなので、横取りする理由が無い。

書く文字列は**矩形と同じ確定の計測から取る**。**確定の計測は矩形だけを返すのではなく、切った `Range` から取り出した選択文も併せて返す** (このアプリでは `pdfTextMatcher.ts` の `getSelectionFromTextLayer` が取り出し、`PdfViewer.tsx` の `popoverState.selectedText` に載って `SelectionPopover.tsx` へ `quote` prop で渡る)。`getSelection().toString()` で取り直してはいけない——この節の前提どおり、そのときページ上の選択はもう畳まれている。

**ガードが 2 つ要る。**どちらも「読者が今コピーしたいのは選んだ passage ではない」場面で、横取りすると読者の操作を壊す。

1. **入力欄自身の中に選択があるとき**は手を出さない (`e.target` が入力要素で `selectionStart !== selectionEnd`)。読者が打った質問文をコピーしている
2. **どこかに生きた選択が残っているとき**も手を出さない (`window.getSelection()` が畳まれておらず、空でもない)。ブラウザ自身がコピーできるものが勝つ、という取り決め——判定は選択の位置を問わない。入力欄が開いている間ページ上の選択は畳まれているので、ここで立つ選択は必ずページの外のものになる。**外側を押すと入力欄が閉じる作り (下記) では、閉じたあとに選ばれた分は購読ごと消えるのでこの経路は狭い**。効いてくるのは入力欄が焦点を取らない実装や、閉じ方が違う実装

**右クリックで入力欄を閉じてはいけない。** 該当するかは閉じる合図に何を購読しているかで決まる——**`mousedown` / `pointerdown` は右ボタンでも来るが、`click` は来ない**。前者で閉じていて、かつ閉じる処理が選択を捨てるなら (この実装ではマウスが選ぶ浮遊ポップオーバーが受け取る `PdfViewer.tsx` の `handlePopoverDismiss` が `removeAllRanges` を呼ぶ。提示バーから開くボトムシート側の `handleQuestionClose` は選択を捨てない)、**コンテキストメニューが出る前に選択が消える**。しかも閉じれば購読も終わる (上記のとおり入力欄の生存期間に閉じてある) ので、**ブラウザの選択と自前の応答が同時に失われ**、メニューからのコピーは何も返さない。閉じるのは主ボタン (`event.button === 0`) だけにする。ガードは入力欄のコンポーネント側に置けば両方の経路に効く。§8 が「送ってよいかは `pointerup` ではなく `pointerdown` の時点で決める」と言うのと同じ `mousedown` 経路の話で、あちらはページ送り、こちらはコピーが巻き添えになる。
...
- **先に別の文字列をコピーしてクリップボードを汚してから**、入力欄が出た状態で Cmd+C を押し、貼り付けたものが選んだ文と一致すること (貼り付け先は入力欄自身でよい)。汚さずに試すと、壊れた実装でも直前のコピーが残っていて合格して見える——それがこの不具合の症状そのもの
...
- **クリップボードは合成イベントと貼り付けで検証する**。jsdom には `ClipboardEvent` も `DataTransfer` も無いので、単体テストでは `new Event("copy", { cancelable: true, bubbles: true })` に `clipboardData` を生やして dispatch し、書かれた値と `defaultPrevented` を見る。**dispatch 先は入力欄自身にする**——`document` へ直接投げると `e.target` が document になり、ガード 1 の分岐を一度も通らないテストになる。E2E では**読み取り API を使わず入力欄へ貼り付けて値を照合する**——`navigator.clipboard.readText()` はドキュメントのフォーカスと権限を要求し、そこで落ちると実装の失敗と見分けがつかない。**貼り付けた値の期待値をその実行で選択した文字列そのものに取ると、ドラッグが 1 文字しか選べていなくても通る**ので、fixture が持つ既知の文字列と突き合わせる (この実装では `SelectionPopover.test.tsx` の 5 件と、`e2e/chatbook.spec.ts` の「copies the passage a reader chose with the question box over it」)
...
| `window.getSelection().isCollapsed` (コピーが空のとき)            | `true` (自前の応答が答える) | `false` (ページ外に残った選択がガード 2 に当たり、横取りが黙って止まる) |
...
| 選択文 (`quote`) | 確定の計測が矩形と併せて返す passage のテキスト (§7) | `pdfTextMatcher.ts` の `getSelectionFromTextLayer` が切った `Range` から取り出し、`PdfViewer.tsx` の `popoverState.selectedText` に載る | `SelectionPopover.tsx` の `copy` 応答、`SelectionActionBar.tsx` の引用表示、ハイライトの保存                                  |

## 依拠する外部事実

いずれも 2026-08-08 に実機とソースで確認。

- 検証したのは当時の `pdfjs-dist` 5.7.284。**その後 6.2.108 へ上がっている** (`package.json` の指定は `^6.2.108`)。§2 の表が挙げる 4 つのカスタムプロパティを `build/pdf.mjs` が今も `setProperty` で書き、`web/pdf_viewer.css` の `.textLayer` が `--text-scale-factor` / `--min-font-size-inv` と同じ `transform` を導出することは 2026-08-19 に確認した (v6 は `--comb-offset` / `--comb-width` も書くが、§2 が写す 3 ブロックの契約は変わらない)。CSS カスタムプロパティ名はバージョン間で変わる (`--scale-factor` 単独で完結していた時期がある)。アップグレード時は同じ 2 箇所を突き合わせること
...
§7 末尾 (コピー) の前提は 2026-08-19 に確認。ブラウザ側の事実は Playwright 1.62.1 の同梱 Chromium (ヘッドレス) で見たもの。

- **ページ上の選択が畳まれていても、フォーカスが編集可能な要素にあれば Cmd+C は `copy` イベントを発火させる**。選ぶものが画面に無いことは、`copy` が来ないことを意味しない——だからこそ購読して答えられる
- **自前の応答を外すと、同じ操作で選んだ文はクリップボードに入らない**。傍受を無効にした状態で E2E を走らせ、貼り付け結果が空になることを確認した (実機の Chrome で読者が見るのは、それ以前にコピーしていた内容がそのまま貼られる形)
- テスト環境の事実: jsdom 29.1.1 には `ClipboardEvent` も `DataTransfer` のコンストラクタも無い (どちらも `undefined`)。単体テストで本物のイベントは組み立てられない
