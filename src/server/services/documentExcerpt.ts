import { bookOutlineSchema, outlineSchema, type BookOutline } from "../../shared/schemas/book";
...
export function readStoredOutline(stored: string | null): BookOutline | null {
  ...
  const compact = bookOutlineSchema.safeParse(parsed);
  if (compact.success) return compact.data;

  // The fork's OCR editor stores the richer nested outline shape. Reduce it to
  // the chapter bounds used for chat while keeping that richer shape available
  // to the reader UI and existing saved books.
  const nested = outlineSchema.safeParse(parsed);
  if (!nested.success) return null;
  const chapters = nested.data
    .filter((entry): entry is typeof entry & { pageNumber: number } => entry.pageNumber !== null)
